...
@Controller
@RequestMapping(value = Constants.URL_API_PAGINATE + ".wadl")
        ...
        try {
			...
			for (Map.Entry<RequestMappingInfo, HandlerMethod> entry : handletMethods
				...
				for (RequestMethod httpMethod : httpMethods) {
					...
					wadlDocMethod.setTitle(javaMethod.getDeclaringClass()
							.getSimpleName() + "." + javaMethod.getName());
					...
					if (!mediaTypes.isEmpty()) {
						...
						ResponseStatus status = handlerMethod
								.getMethodAnnotation(ResponseStatus.class);
						if (status == null) {
							wadlResponse.getStatus().add(
			...
			log.error("Problem generating WADL",e);
		}
